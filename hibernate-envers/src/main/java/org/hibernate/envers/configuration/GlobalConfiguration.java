...
public class GlobalConfiguration {
    ...
    public GlobalConfiguration(ServiceRegistry serviceRegistry) {
		final ConfigurationService configurationService = serviceRegistry.getService( ConfigurationService.class );

        String generateRevisionsForCollectionsStr = getProperty(configurationService,
        ...
        String ignoreOptimisticLockingPropertyStr = getProperty(configurationService,
		...
		String storeDataDeletedEntityStr = getProperty(configurationService,
		...
		defaultSchemaName = configurationService.getSetting( "org.hibernate.envers.default_schema", String.class, null );
		...
		correlatedSubqueryOperator = "org.hibernate.dialect.HSQLDialect".equals(
				configurationService.getSetting( "hibernate.dialect", String.class, null )
		) ? "in" : "=";

        String trackEntitiesChangedInRevisionEnabledStr = getProperty(configurationService,
        		"org.hibernate.envers.track_entities_changed_in_revision",
        		"org.hibernate.envers.track_entities_changed_in_revision",
        		"false");
        trackEntitiesChangedInRevisionEnabled = Boolean.parseBoolean(trackEntitiesChangedInRevisionEnabledStr);

        String useRevisionEntityWithNativeIdStr = getProperty(configurationService,
		...
		hasGlobalSettingForWithModifiedFlag =
				configurationService.getSetting( GLOBAL_WITH_MODIFIED_FLAG_PROPERTY, String.class, null ) != null;
		String usingModifiedFlagStr = getProperty(
				configurationService, GLOBAL_WITH_MODIFIED_FLAG_PROPERTY, GLOBAL_WITH_MODIFIED_FLAG_PROPERTY,
				"false"
		);
		globalWithModifiedFlag = Boolean.parseBoolean( usingModifiedFlagStr );

		modifiedFlagSuffix = getProperty(
				configurationService, MODIFIED_FLAG_SUFFIX_PROPERTY, MODIFIED_FLAG_SUFFIX_PROPERTY,
				DEFAULT_MODIFIED_FLAG_SUFFIX
		);

		String revisionListenerClassName = configurationService.getSetting(
...
//                revisionListenerClass = (Class<? extends RevisionListener>) Thread.currentThread().getContextClassLoader().loadClass(revisionListenerClassName);
			// TODO: TEST ME
				revisionListenerClass = serviceRegistry.getService( ClassLoaderService.class ).classForName( revisionListenerClassName );
